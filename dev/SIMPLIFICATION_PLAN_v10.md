...
| TM-01 | READ | `master.zig:126-187` | `fillHeights` conflates pin pass + two distributions | extract `pinCapped` (+10 net) [~ no golden test on pin path] — **KEEP** per owner ruling 2026-09-25: pin pass documented by the three-scheme comment at :161-166, no golden test, +10 not a reduction | +10 | M |
...
|---|---|---|---|
| Q-01 | **C-16** `parser.lastScalar` conflates literal arrays with accumulation. Observable today: `border_focused = [#aa0000, #008800]` yields solid `#008800` (last element), never the documented 50/50 bare-list mix; duplicate palette-*name* declarations average instead of later-wins. | Two documented features share one shape; which wins is a design decision with test pins on both sides. | **RESOLVED 2026-09-24: applied** — `Value.array` gains `accumulated`; literal arrays stay mixes, accumulated duplicates stay later-wins (see §9). |
| Q-02 | **ST-01** title.zig memo removal (−110 LoC). Output-identical, but `SegmentedTitlesMemo` exists so a scrolling split-view doesn't re-gather + re-measure every frame. | Cuts a *deliberate* perf capability on the scroll path (Pango shapes/caches absorb most cost per the audit, but the gather is O(k log k) per frame). | **RESOLVED 2026-09-24: applied** — memos removed, gather per-frame, scroll-path notes kept (see §9). |
| Q-03 | **B-01** `RightCluster` (−25..−30). Keeps a per-frame Pango re-measure for right segments; falls back to re-measure for >16 anyway. | Behavior-identical but touches the hottest geometry path in the bar; the audit rated it MED risk. | **RESOLVED 2026-09-24: applied** — inlined at both call sites (see §9). |
| Q-04 | **P-02 / P-05** slider `Level` removal + pct↔range consolidation. | The `Level` struct is the only place the "commit-then-reread" apply discipline is documented centrally; the pct↔range formulas are pinned by two test suites and previously owner-deferred pending a rounding ruling. | **RESOLVED 2026-09-24: applied** — `Level` deleted, single nearest-round map (see §9). |
| Q-05 | **U-04** `ids.fromIndex(anytype)` (−14 casts). | The `u8` param is *documented as intentional* ("prevents confusing indices"); widening to `anytype` trades Debug compile-error for ReleaseFast silent truncation. | **RESOLVED 2026-09-24: applied** — widened, checked cast centralized, tests wired into `ids_test` (see §9). |
| Q-06 | **F-02** tracking `workspace_count` mirror → config-direct latch; drop modules' init/deinit bindings. | Touches module-registry wiring (deletion of bindings) and the teardown `0` latch; the audit rated MED. | **RESOLVED 2026-09-24: applied** — config latch with init-time `core.isReady()` guard (see §9). |
| Q-07 | **W-04** `detachTiledToFloating` leaves a stale `tiled_order` membership contradicting its own comment. | This is arguably a latent bug fix (unify on `removeValue`), not pure simplification. | **RESOLVED 2026-09-24: applied** — membership now really dropped via `removeValue`; X-gated actions_test verified under Xvfb (see §9). |
| Q-08 | **B-14** multi-family Pango font "fallback support" is first-family-only. | Fixing it is a *behavior change* (log + feature honesty). | **RESOLVED 2026-09-24: applied** — pre-joined family list passed through to Pango; docs reconciled (see §9). |
| Q-09 | **IN-08** three XKB retry loops → comptime `withRetries`. | `anytype` closure plumbing may read worse than the ~10 lines it removes; audit split judgment. | **RESOLVED 2026-09-24: applied** — single-shot named-arg probes, net −8..−12 (see §9). |
| Q-10 | **K-01 / S-03** pipeline grab-pair fold + `reconcileUnderGrab` fold. | Ordering semantics are the load-bearing part of the focus/latency path; each fold touches 7-8 call sites incl. test fixtures and moves the RETILE_PROF home. | **RESOLVED 2026-09-24: K-01 applied**; S-03 NOT applied (KEEP, RETILE_PROF home per K-17) (see §9). |
| Q-11 | **B-17 / LV-04 / T-04 / T-01 / FM-01 / FM-03/FM-07** medium-risk structural items (widthState key, segdraw content mode, moduleOf seam, overflow-share head fallback, resizeCorner merge, single pending-bar global). | Each trades several lines/API surface in a module with no golden test on the touched path. | **RESOLVED 2026-09-25: all applied** (B-17 keyedWidthState, LV-04 drawAndStore, T-04 moduleOf seam, T-01 emitOverflowShare head-fallback, FM-01 nearestResizeCorner, FM-03+FM-07 g_pending_bar). Also in this batch: TM-01 fillHeights pinCapped — **KEEP** per owner ruling 2026-09-25 (pin pass documented at :161-166, no golden test, +10 not a reduction), see §9 rows 448-453. |
...
| 2026-09-24 | config | C-01..C-13, C-15, CF-01..CF-07 (C-14 skipped: restructure duplicates the dispatch switch, net-negative; C-16 deferred Q-01 → applied 2026-09-24, see §9) | check ✓ / tests ✓ | −~75 |
| 2026-09-24 | core | U-01..U-03, U-05..U-07, U-11..U-13, U-16 (U-04 deferred Q-05 → applied 2026-09-24, see §9; U-15 skipped: net-zero helper, no clear win); K-02..K-12 (K-01 deferred Q-10 → applied 2026-09-24, see §9); S-01, S-02, S-04..S-14 (S-03 deferred Q-10 → KEEP, RETILE_PROF home per K-17); X-05, X-12; model.zig Store via utils facade, bounded import dropped (X-04) | check ✓ / tests ✓ | verified |
| 2026-09-24 | window | W-01..W-03, W-05..W-15 (W-04 deferred Q-07 → applied 2026-09-24, see §9; W-16 skipped: named atom lookups clearer than a table; W-18 KEEP: documented alias layer); X-09, X-10; F-01, F-03..F-12 (F-02 deferred Q-06 → applied 2026-09-24, see §9; F-04 folded fallback into `sendBorderColorIfChanged`; F-07/F-11/F-12 tracking header rewrites); FM-02, FM-04, FM-05, FM-06, FM-08..FM-10, FM-12 (FM-01/FM-03 deferred Q-11 → applied 2026-09-25, see §9; FM-07 folded into FM-03; FM-11 folded into F-08) | check ✓ / tests ✓ | verified |
| 2026-09-24 | tiling | T-02 (clampAspectDim max-cap drop), T-03 (layoutKindOf deleted; callers → layoutKindFallingBack), T-05..T-09 (layoutModule doc, paneCell→grid, compute n=0 doc, cycleKind guard doc, registry via contract), TM-02 (zero_boost weight branch constant), TM-03 (dead `else 0` arms), TM-04 (space_per_window u16 + rowPitch), TM-05..TM-12 (recursive→spiral prose, monocle utils import, scroll casts/comment, min_region, axis-nest, leaf type-dance, cellStride) (T-01/T-04 deferred Q-11 → applied 2026-09-25, see §9; TM-01 → **KEEP** per owner ruling 2026-09-25: pin pass already documented, no golden test, +10 not a reduction) | check ✓ / tests ✓ | verified |
| 2026-09-24 | input | IN-01 (reload XKB guard deleted; boot/shutdown-only docs), IN-02 (`*anyopaque` → `core.Connection`; FFI boundary keeps 4 `@ptrCast` — xkbcommon-x11's own cimport opaque), IN-03 (history prose condensed in input/keysyms/xkbcommon), IN-04 (`masks.isModifierKeysym`; prompt too), IN-06 (never-firing buf guard), IN-07 (uniform `getXkbState`), IN-08 deferred Q-09 → applied 2026-09-24, see §9, IN-09 (`dirSignFloat`), IN-10 (keysymToKeycode doc trim), IN-11 (name_buf why-comment; xkbcommon thresholds already named consts), IN-12 (resolver-ownership → pointer to keybind) | check ✓ / tests ✓ | verified |
...
| 2026-09-24 | bar | B-02 (drawPaddedSegmentValue no-split fallback → paintedSegment), B-03 (hasLayoutSegmentDirty direct loop), B-04 (BarSetup.dc dropped), B-05 (segAt doc fixed), B-06 ("live state" doc → State), B-07 (centerRowBudget extracted), B-08 (→ contract.Segment), B-09 (gather slice + folded clamp), B-10 (cached_metrics tuple), B-11 (bar-height consts + clampBarHeight → scale.zig), B-12 (foldModuleRedraw ×3), B-13 (named is_right_click), B-15 (probeMetrics → FontMetrics directly), B-16 (titleIdBound); B-01 deferred → applied 2026-09-24, see §9, B-14 deferred → applied 2026-09-24, see §9 [behavior note: font fallback keeps first family] | check ✓ / tests ✓ | verified |
| 2026-09-24 | bar | ST-02 (naturalWidthFor deleted; fact onto g_slot_width), ST-03 (emptyWorkspace inlined), ST-04 (batt direct bufPrint), ST-05 (marquee shared ellipsis tail), ST-06 (systatus essay cut), ST-07 (render_buf_len), ST-08 (segmentBounds/segmentIndexOfX → segmod), ST-09 (gather clamp, with B-09); ST-01 deferred Q-02 → applied 2026-09-24, see §9 | check ✓ / tests ✓ | verified |
...
| 2026-09-24 | Q-deferred | IN-08 (three XKB retry loops → one comptime `withRetries(T, args, attempt, failure)` absorbing the try→`retryDelay`→error skeleton; `setupOnce`/`deviceOnce`/`keymapOnce` become single-shot probes reporting "not ready" as null; keymap arg-pair hoisted to a named `keymapOnceArgs` struct; net −8..−12, verified identical behavior under xtest.sh) | check ✓ / layers ✓ / 286 tests ✓ | verified |
| 2026-09-24 | Q-deferred | K-01 (`reconcileGrabFocus` + `reconcileUnderGrabNowWithFocusDuty` folded into one `reconcileGrabFocus(o, t, order, duty)`; duty runs inside the grab after the focus protocol when order is `.before` — the viewport-snap fold; 7 call sites pass `null`, focus.zig carries the real duty; S-03 reconcileUnderGrab fold NOT applied, per K-17 KEEP verdict: RETILE_PROF home stays in sync) | check ✓ / layers ✓ / 286 tests ✓ | verified |
| 2026-09-25 | Q-11 | T-01 (`emitOverflowShare` head-fallback fold: `top` param dropped — the focusedElse fallback to the list head is folded inside `emitOverflowShare(ctx, windows, r)`; leaf.zig + fibonacci.zig callers slimmed) | fmt ✓ / check ✓ / layers ✓ / 286 tests ✓ | verified |
| 2026-09-25 | Q-11 | T-04 (`moduleOf` seam: `contract.moduleOf(kind) ?*const Layout` over `tiling_mods`; `activeLayoutKind` + tiling.zig moduleName/variantCount/compute + pipeline.zig preReconcileDuties + actions.zig viewportContext/resolveVariant/fifo + persist.zig:384 all read the seam; per-module `tiling_mods` aliases dropped where unused) | fmt ✓ / check ✓ / layers ✓ / 286 tests ✓ | verified |
| 2026-09-25 | Q-11 | FM-01 (`ResizeDirection` + `resizeDirectionFromPoint` + `nearestCorner` merged into single `nearestResizeCorner(x, y, rect, border_width) ResizeCorner`; `snapAxis` restored after an edit shadow-removed it) | fmt ✓ / check ✓ / layers ✓ / 286 tests ✓ | verified |
| 2026-09-25 | Q-11 | FM-03 + FM-07 (fullscreen pending-bar hide/show: two module globals → single `g_pending_bar: ?PendingBar{ win, hide }`; `resetState`/`notifyConfigureIfPending`/`armPendingBarHide`/`armPendingBarShow`/`resolvePendingBarShow`/`onWindowGone` unwound — the wait-for-configure-on-disown no-op branch folded out per FM-07) | fmt ✓ / check ✓ / layers ✓ / 286 tests ✓ | verified |
| 2026-09-25 | Q-11 | LV-04 (`segdraw.drawAndStore(name, dc, config, height, start_x, text)` — draw + widthState.store in one; layout.zig/variants.zig file-scope `widthState` instantiation deleted, draw became one-liners; variants' empty-indicator 0-width/store-0 preserved; segdraw gains `drawing`/`types` imports, no cycle) | fmt ✓ / check ✓ / layers ✓ / 286 tests ✓ | verified |
| 2026-09-25 | Q-11 | B-17 (`segdraw.keyedWidthState(tag, Key)` — clock.zig's `reserved_width`/`reserved_mode` own tracker replaced by a keyed width state: `matches(key)` drives once-per-mode re-probe, `naturalWidth` keeps the fallback-to-probe semantics, `invalidate` = reload re-measure) | fmt ✓ / check ✓ / layers ✓ / 286 tests ✓ | verified |
| 2026-09-25 | Q-11 | TM-01 — **KEEP** per owner ruling: `fillHeights` pin pass stays conflated with the two distributions; the three-scheme comment at master.zig:161-166 documents the split, no golden test on the pin path, `pinCapped` extraction +10 net is not a reduction | verdict only | — |